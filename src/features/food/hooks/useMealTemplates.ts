...
export function useMealTemplates() {
  ...
  const { data, loading, refetch } = useQuery(GET_USER_MEAL_TEMPLATES, {
  ...
  const templates: MealTemplate[] = data?.meal_templatesCollection?.edges?.map((edge: any) => edge.node) || [];
  ...
  const toggleFavorite = useCallback(async (templateId: string, currentFavorite: boolean) => {
  ...
  const logTemplate = useCallback(async (template: MealTemplate) => {
    // Increment use count
  ...
  const getTemplatesByMealType = useCallback((mealType: string) => {
