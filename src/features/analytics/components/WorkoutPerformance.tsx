...
export function WorkoutPerformance({ workoutCalendar, workoutStats, weeklyEffort }: Props) {
  ...
  weeks.push(workoutCalendar.slice(-30))
            ...
            {weeks.map((week, weekIdx) => (
              <div key={weekIdx} className="grid grid-cols-7 gap-2">
                {week.map((day, dayIdx) => (
                  <div
                    key={dayIdx}
                    className={`aspect-square rounded-lg border-2 flex flex-col items-center justify-center p-1 cursor-pointer transition-all hover:scale-105 ${day.workoutType ? "border-primary bg-primary/5" : "border-gray-200 bg-gray-50"
                      } ${getIntensityColor(day.caloriesBurned)}`}
                    title={
                      day.workoutType
                        ? `${formatDate(day.date)}: ${day.workoutType}\n${day.duration}min, ${day.caloriesBurned
                        }cal`
                        : formatDate(day.date)
                    }
                  >
