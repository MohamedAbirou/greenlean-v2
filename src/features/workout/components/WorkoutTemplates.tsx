...
export function WorkoutTemplates({ onTemplateSelect }: WorkoutTemplatesProps) {
    ...
    const handleToggleFavorite = async (templateId: string, currentStatus: boolean) => {
        try {
            ...
            setTemplates(templates.map(t =>
                t.id === templateId ? { ...t, is_favorite: !currentStatus } : t
            ));
    ...
    const filteredTemplates = (activeTab === 'favorites' ? favoriteTemplates : templates)
        .filter(template => {
            ...
            const matchesMealType = selectedWorkoutType === 'all' || template.workout_type === selectedWorkoutType;
            ...
            <div className="flex items-center gap-3 overflow-x-auto pb-2">
                ...
                <div className="flex gap-2">
                    <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${selectedWorkoutType === 'all'
                    ...
                    {Object.entries(workoutTypeConfig).map(([type, config]) => (
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${selectedWorkoutType === type
                                ? 'bg-primary-500 text-white'
                                : 'bg-muted hover:bg-muted/80'
                                }`}
                        >
                                    ...
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex flex-wrap gap-2 mb-3">
                                                ...
                                                {template.difficulty_level && (
                                                    <Badge className={`${getDifficultyColor(template.difficulty_level)} text-white capitalize`}>
                                                        {template.difficulty_level}
                                                    </Badge>
                                                )}
                                        ...
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                >
                                                    <Heart className={`h-4 w-4 ${template.is_favorite ? 'fill-current text-red-500' : ''}`} />
                                                </Button>
                                                <Button
                                                    variant="ghost"
                                                    size="sm"
                                        ...
                                        <div className="mt-2 space-y-1">
                                            {template.exercises.map((exercise, idx) => (
                                                <div key={idx} className="text-sm text-muted-foreground pl-4">
                                                    {idx + 1}. {exercise.name}
                                                    <details className="mb-3 py-2 border-b border-border">
                                                        <summary className="cursor-pointer text-xs font-medium text-muted-foreground hover:text-accent-500">
                                                            View sets ({exercise.sets.length})
                                                        </summary>
                                                        <div className="mt-2 space-y-1">
                                                            {exercise.sets.map((set, setIdx) => (
                                                                <div key={setIdx} className="text-sm text-muted-foreground pl-4">
                                                                    {set.set_number}. {set.reps} {set.weight_kg ? ` @ ${set.weight_kg}kg` : ''}
                                                                </div>
                                                            ))}
                                                        </div>
                                                    </details>
