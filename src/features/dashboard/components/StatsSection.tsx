...
import { WeightTracking } from "@/features/analytics/components/WeightTracking";
...
export function StatsSection() {
  ...
  const defaultStart = format(subDays(new Date(), 29), 'yyyy-MM-dd'); // Last 30 days
  const [startDate, setStartDate] = useState(defaultStart);
  const [endDate, setEndDate] = useState(defaultEnd);

  const { data: statsData, isLoading, error } = useStatsData(user?.id, startDate, endDate);
  ...
  return (
    <div className="space-y-12">
      ...
      <WeightTracking startDate={startDate} endDate={endDate} />
